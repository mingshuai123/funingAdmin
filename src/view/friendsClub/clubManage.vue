          ...
          <div class="item" v-for="(item,index) in memberList" :key='index' >
            <div class="memberinfo">
              ...
              <div>
                ...
                <div style="font-size:12px;color:#868686;">ID : {{item.uid}}</div>
              </div>
            </div>
            <div class="date">{{item.created.slice(2, 16)}}</div>
            <div class="handle">
              <van-button size='small' type="danger" v-if='!item.admin' @click="kickout(item.nickname,item.uid)" style="margin-right:5px">踢出</van-button>
              <van-button size='small' type="info" @click='checkmember(item.uid)'>查看</van-button>
            </div>
          </div>
        </div>
      ...
      <van-tab title="加入申请">
        <div class="memberlist">
          ...
          <div class="item" v-for="(item,index) in memberList" :key='index' >
            ...
            <div class="date">{{item.created.slice(2, 16)}}</div>
            <div class="handle">
              <van-button size='mini' >踢出</van-button>
...
export default {
  ...
  methods:{
        ...
        if(res.errno==0){
          _this.$dialog.confirm({
            ...
            var data={
              ...
              club_id:_this.clubInfo.normal_club_id,
              uid: _this.searchId,
              club_type: 'NORMAL_CLUB'
            }
    ...
    checkmember(id){
      this.$router.push({path:'/playerManage',query:{uid:id}});
    },
    kickout(nickname,id){
      var _this=this;
      this.$dialog.confirm({
