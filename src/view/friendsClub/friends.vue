<template>
  <div class="player">
    ...
    <div class="pageclick">
      <van-button type="info" size='small' @click="$router.push({path:'/clubManage'})">管理</van-button>
      <van-button type="info" size='small' @click="$router.push({path:'/matchManage'})">比赛</van-button>
      <van-button type="info" size="small" @click="$router.push({path:'/statistics',query:{type:'friends'}})">统计</van-button>
    </div>
...
export default {
  ...
  methods:{
    ...
    getInfo(){
      this.postAxios({method: 'check.club',club_id: this.clubInfo.normal_club_id}).then(res=>{
        this.info=res;
      })
    },
    getlist(){
      var data={
        ...
        uid: this.clubInfo.normal_club_id,
        page: this.page,
        u_type: 'CLUB',
        r_type: ''
      }
